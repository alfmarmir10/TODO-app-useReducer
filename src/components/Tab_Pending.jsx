...
const Tab_Pending = () => {
  ...
  return (
    <div className="width-100percent tab-main-container overflow-y-auto">
      {
        (tasksKeys) && tasksKeys.map((key)=>{
          if(tasks[key]["Status"]==="Pending"){
            return (
              <TaskCard description={tasks[key]["Description"]} key={Number(Math.random() * (1000 - 0) + 0)} itemKey={key} filter="Pending"/>
            )
          } else {
